# %%
"""
Experiment Script for Online Inference with Transformer Model
...
def experiment_online_inference(config: dict):
    """
    Run online inference experiment for transformer model.
    ...
    print(f"\n{'='*60}")
    print(f"Online Inference Experiment")
    print(f"{'='*60}")
    ...
    backbone_model = ITETransformer(config=config)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run online inference for Transformer model")
    parser.add_argument(
        "--develop_set",
        type=str,
