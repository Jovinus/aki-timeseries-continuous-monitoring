# %%
"""
Online Inference Module for 1D CNN Model (Optimized with Batching & DataLoader)
...
- Handles fixed-length input requirement of 1D CNN with left-padding
...
- OPTIMIZED: Supports cross-patient batching for maximum GPU utilization
...
class OnlineInferenceDataset1DCNN(Dataset):
    """
    Dataset for online inference that loads patient data and prepares
    all timestamp samples for a patient (1D CNN version).
    
    Each item returns all cumulative data tensors for a single patient,
    along with metadata and demographic features.
    """
    ...
    def __init__(
        ...
        data_dir: str,
        time_column_idx: int = -1,
        time_resolution: float = 1.0,
        min_observations: int = 1,
    ):
        """
        Args:
            meta_table: DataFrame with 'visit_id', 'label', 'age', 'sex', 'bmi' columns
            data_dir: Directory containing patient data files (.gz)
            time_column_idx: Index of time column in data (-1 to use row index)
        ...
        """
        self.meta_table = meta_table.copy().reset_index(drop=True)
        # Convert sex if needed
        if self.meta_table["sex"].dtype == object:
            self.meta_table["sex"] = self.meta_table["sex"].map({"M": 0, "F": 1})
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary with:
            - 'data_tensors': List of tensors, each shape (seq_len, num_features)
            - 'meta_tensor': Demographic features tensor
            - 'visit_id': Patient visit ID
            - 'label': Ground truth label
            - 'timestamps': List of timestamps
            - 'n_observations': List of observation counts
        """
        row = self.meta_table.iloc[idx]
        visit_id = row["visit_id"]
        label = row["label"]
        meta = row[["age", "sex", "bmi"]].to_numpy(dtype=float)
        ...
        # Extract timestamps
        if self.time_column_idx >= 0 and self.time_column_idx < data.shape[1]:
            timestamps = data[:, self.time_column_idx]
        else:
...
def online_inference_collate_fn_1dcnn(batch: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    Custom collate function for 1D CNN online inference.
    Flattens samples from multiple patients but does NOT apply the collate function
    here - that's done in chunks during inference to avoid OOM.
    """
    all_data_tensors = []
    all_meta_tensors = []
    all_metadata = []
    ...
    for item in batch:
        for i, data_tensor in enumerate(item['data_tensors']):
            ...
            all_meta_tensors.append(item['meta_tensor'])
            all_metadata.append((
                item['visit_id'],
                item['label'],
                item['timestamps'][i],
    ...
    return {
        ...
        'meta_tensors': all_meta_tensors,  # List of meta tensors
...
def run_online_inference_dataloader(
    ...
    """
    Run online inference using PyTorch DataLoader for parallel data loading (1D CNN).
    ...
    Args:
        ...
        seq_len: Expected sequence length for 1D CNN model
        time_column_idx: Index of time column in data (-1 to use row index)
        time_resolution: Time resolution for grouping timestamps (in hours)
        min_observations: Minimum observations required for prediction
    ...
    inference_collate_fn = CollateFn1dCnnWithResolutionControl(
        method=None,
        resolution=None,
    ...
    # Create dataset
    dataset = OnlineInferenceDataset1DCNN(
    ...
    # Create DataLoader (collate_fn does NOT apply 1D CNN collation - just flattens)
    dataloader = DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=False,
        num_workers=num_workers,
        collate_fn=online_inference_collate_fn_1dcnn,
    ...
    with torch.no_grad():
        for batch in iterator:
            ...
            meta_tensors = batch['meta_tensors']
            ...
            for chunk_start in range(0, len(data_tensors), gpu_batch_size):
                ...
                chunk_data = data_tensors[chunk_start:chunk_end]
                chunk_meta = meta_tensors[chunk_start:chunk_end]
                
                # Apply collate function to this chunk
                dummy_labels = [torch.LongTensor([0]) for _ in chunk_data]
                batch_items = list(zip(chunk_meta, chunk_data, dummy_labels))
                collated = inference_collate_fn(batch_items)
                
                collated_data = collated['data'].to(device_obj)
                collated_meta = collated['meta'].to(device_obj)
                
                # Forward pass
                logits = lightning_module.backbone_model(collated_meta, collated_data)
                
                # Model may already apply softmax
                if logits.sum(dim=-1).mean() < 0.99 or logits.sum(dim=-1).mean() > 1.01:
                ...
                # Free GPU memory
                del collated_data, collated_meta, logits
...
class OnlinePredictor1DCNN:
    """
    Online predictor for 1D CNN that generates predictions at each timestamp.
    ...
    The 1D CNN model requires fixed-length input, so data is padded/truncated.
    ...
    def __init__(
        ...
        Args:
            lightning_module: Trained PyTorch Lightning module
            scaling_info: DataFrame with normalization parameters (feature, median, iqr)
                          Includes both time-series features and demographics (age, bmi)
        ...
        self.collate_fn = CollateFn1dCnnWithResolutionControl(
            method=None,  # No resolution control during inference
            resolution=None,
            scaling_info=scaling_info,
            apply_prob=0.0,  # No augmentation during inference
            seq_len=seq_len,
        )
    ...
    def predict_single_timestamp(
        ...
        Args:
            data: Input tensor of shape (seq_len, num_features)
            meta: Demographic tensor of shape (num_demo_features,)
        ...
        # Apply collate function (handles padding, normalization, nan_to_num)
        batch = self.collate_fn([(meta, data, dummy_label)])
        
        collated_meta = batch["meta"].to(self.device)
        collated_data = batch["data"].to(self.device)
        
        # Forward pass - note: 1D CNN expects (meta, data)
        logits = self.lightning_module.backbone_model(collated_meta, collated_data)
        
        # Model already applies softmax, but let's ensure
        if logits.sum(dim=-1).mean() < 0.99 or logits.sum(dim=-1).mean() > 1.01:
            probs = F.softmax(logits, dim=-1)
        else:
            probs = logits
    ...
    def predict_batch(
        ...
        Args:
            data_list: List of input tensors, each of shape (seq_len, num_features)
            meta_list: List of meta tensors, each of shape (num_demo_features,)
        ...
        batch_items = list(zip(meta_list, data_list, dummy_labels))
    ...
    def predict_online(
        ...
        Args:
            data_dict: Dictionary containing 'data' key with numpy array of shape (seq_len, num_features)
            meta: Demographic features array of shape (num_demo_features,)
            time_column_idx: Index of time column in data (default 0 for first column)
                            Set to -1 if data doesn't have explicit time column (use row index)
            time_resolution: Time resolution for grouping timestamps (in hours)
        ...
        """
        data = data_dict["data"]  # Shape: (seq_len, num_features)
        
        # Extract timestamps
        if time_column_idx >= 0 and time_column_idx < data.shape[1]:
            timestamps = data[:, time_column_idx]
        else:
            # Use row indices as timestamps (assumes uniform time steps)
            timestamps = np.arange(len(data), dtype=float)
        ...
        for i in range(0, len(data_tensors), self.batch_size):
            batch_data = data_tensors[i:i + self.batch_size]
            batch_meta = meta_tensors[i:i + self.batch_size]
            batch_probs = self.predict_batch(batch_data, batch_meta)
...
def run_online_inference(
    ...
    **kwargs,  # Accept extra kwargs for compatibility
    ...
    """
    Run online inference for all patients in the meta table.
    
    OPTIMIZED: Uses batched inference for timestamps within each patient.
    ...
    Returns:
        Dictionary mapping visit_id to DataFrame of online predictions
    """
    predictor = OnlinePredictor1DCNN(
    ...
    # Preprocess meta table (convert sex)
    meta_table = meta_table.copy()
    if meta_table["sex"].dtype == object:
        meta_table["sex"] = meta_table["sex"].map({"M": 0, "F": 1})
    
    results = {}
    
    iterator = meta_table.iterrows()
    if show_progress:
        iterator = tqdm(iterator, total=len(meta_table), desc="Online Inference")
...
def run_online_inference_batched_patients(
    ...
    for batch_start in iterator:
        ...
        patient_batch = meta_table.iloc[batch_start:batch_end]
        ...
        for _, row in patient_batch.iterrows():
            ...
            for current_time in unique_timestamps:
                ...
                data_tensor = torch.from_numpy(cumulative_data).float()
                all_data_tensors.append(data_tensor)
                all_meta_tensors.append(meta_tensor)
        ...
        with torch.no_grad():
            for i in range(0, len(all_data_tensors), batch_size):
                batch_data = all_data_tensors[i:i + batch_size]
                batch_meta = all_meta_tensors[i:i + batch_size]
                dummy_labels = [torch.LongTensor([0]) for _ in batch_data]
                batch_items = list(zip(batch_meta, batch_data, dummy_labels))
                batch = collate_fn(batch_items)
                
                collated_meta = batch["meta"].to(device_obj)
...
def aggregate_online_results(
    ...
    Args:
        ...
        meta_table: Optional DataFrame with patient metadata including
    ...
    # If meta_table is provided, calculate time differences
    if meta_table is not None:
        # Merge with meta table to get datetime columns
        datetime_cols = ["visit_id"]
        optional_cols = ["admission_dt", "discharge_dt", "aki_dt", "los", "loo"]
        for col in optional_cols:
            if col in meta_table.columns:
                datetime_cols.append(col)
        
        meta_subset = meta_table[datetime_cols].drop_duplicates(subset=["visit_id"])
        all_results = all_results.merge(meta_subset, on="visit_id", how="left")
...
if __name__ == "__main__":
    # Example usage
    print("Online Inference Module for 1D CNN Model (Optimized)")
    print("=" * 60)
    print("\nOptimizations:")
    ...
    print("\nUsage:")
    print("  from scripts.online_inference import run_online_inference_dataloader")
    print("  ")
    ...
    print("  from scripts.online_inference import run_online_inference_batched_patients")
