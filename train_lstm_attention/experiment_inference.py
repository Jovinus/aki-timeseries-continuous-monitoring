# %%
"""
Inference Experiment Script for LSTM with Attention Model

This script runs inference on trained LSTM Attention models
for internal test set and external validation sets.

Usage:
    python experiment_inference.py --develop_set ilsan --prediction_window_size 48
"""
...
def experiment_holdout(
    ...
    """
    Run inference experiment with holdout validation.
    
    This function:
    1. Loads trained model checkpoint
    2. Runs inference on development test set
    3. Runs inference on external validation sets
    4. Saves predictions
    """
    set_seed(1004)
    ...
    print(f"\n{'='*60}")
    print(f"Inference - LSTM with Attention Model")
    print(f"{'='*60}")
    ...
    # Internal Test Set
    # =========================================================================
    print(f"--- Development Test Set ({config['develop_set']}) ---")
    ...
    print(f"Saved predictions: {len(df_test)} samples")
    ...
    # =========================================================================
    # External Validation Set (cchlmc or ilsan)
    # =========================================================================
    ...
    print(f"Saved predictions: {len(df_external)} samples")
    ...
    print(f"Results saved to: {config['save_dir_predictions']}/{config['develop_set']}/{config['exp_nm']}/{config['version']}/")
    print(f"{'='*60}\n")
    
def parse_args():
    parser = argparse.ArgumentParser(description="Run inference for LSTM with Attention model")
