# %%
"""
Online Inference Module for LSTM Attention Model (Optimized with Batching & DataLoader)
...
def online_inference_collate_fn(
    ...
    Does NOT apply the LSTM collate function here - that's done in chunks
...
def run_online_inference_dataloader(
    ...
    if scaling_info is not None:
        inference_collate_fn = partial(collate_fn_lstm_attention, scaling_info=scaling_info)
    else:
        inference_collate_fn = collate_fn_lstm_attention
    ...
    # Create DataLoader (collate_fn does NOT apply LSTM collation - just flattens)
...
class OnlinePredictor:
    ...
    def __init__(
        ...
        if scaling_info is not None:
            self.collate_fn = partial(collate_fn_lstm_attention, scaling_info=scaling_info)
        else:
            self.collate_fn = collate_fn_lstm_attention
...
def run_online_inference_batched_patients(
    ...
    if scaling_info is not None:
        ...
        collate_fn = collate_fn_lstm_attention
...
if __name__ == "__main__":
    # Example usage
    print("Online Inference Module for LSTM Attention Model (Optimized)")
    print("=" * 60)
    print("\nOptimizations:")
